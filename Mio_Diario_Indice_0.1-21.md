...
* **Rendering progressivo** — all’apertura carica una sola volta i metadati indicizzati del Diario e renderizza soltanto il primo batch; le schede successive vengono costruite durante lo scrolling.
...
function widgets.IndiceDiario()
  ...
  -- I metadati Diario vengono letti soltanto dall’indice SilverBullet.
  -- Nessun corpo Markdown e nessun documento media viene letto.
...
Rimossa dall'Indice ogni gestione delle fotografie e delle gallerie, ora demandata interamente a `Mio_Diario_PhotoGallery` e `PhotoGateway`.

Modifiche principali:

* eliminata la query globale su `index.documents()` e la scansione dei documenti `media/`;
* rimosse `indiceDiarioIsImageExtension()`, `indiceDiario.galleryDates()` e `indiceDiarioEntryDayKey()`;
* rimosso il link `📷 Galleria` dalle schede;
* eliminati `galleryDates`, `entryNodes` e il secondo `setTimeout()` usato per arricchire le schede dopo il primo rendering;
* il widget carica ora esclusivamente i metadati indicizzati delle pagine Diario tramite `index.subPages("Diario")`;
* mantenuti invariati rendering progressivo, filtro, tags, luoghi, Viaggio, riepiloghi e Virtual Page;
* nessuna chiamata a `PhotoGateway` viene introdotta nell'Indice: la gestione fotografica resta separata nella libreria dedicata.

La responsabilità dell'Indice torna quindi a essere esclusivamente la lettura, il filtro e la visualizzazione dei metadati del Diario.
